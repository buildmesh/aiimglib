...
- `GET /api/images/{id}` – detail view including raw `prompt_meta`, resolved references, list of dependent media (children whose prompts reference this record), and media URLs for both `file_name` and `thumbnail_file`.
...
- Tiles now open a richer detail modal: inline HTML5 video playback for `media_type=video`, image zoom for images, metadata, rating, and lists of both referenced sources (upstream) and dependent media (downstream assets whose prompts reference the current record). Reference/dependent thumbnails are clickable, navigating to those entries.
